...
const AddInspector = (props) => {
    const [formAddInspectorToPlan,setFormAddInspectorToPlan]= useState({id:props.planId})
    const {dataGetAllMods,errorGetAllMods,loadingGetAllMods}= useGetAllMods()
    const {errorAddInspectorToPlan,executeAddInspectorToPlan}= useAddInspectorToPlan()
    const [state]= useContext(StoreContext)
    const handleAddInspectorToPlan=async(e)=>{
        e.preventDefault()
        await executeAddInspectorToPlan(formAddInspectorToPlan,state.id,state.token)
      ...
      return (
        ...
        <div className='add-inspector-container'>
            <div className="add-inspector-content">
                ...
                >
                  <FaTimes 
                   onClick={()=>{props.handleShowModalAddInspectorToPlan(false)}}
                  />
                </div>
                <form action="">
                   <label htmlFor="Chọn thanh tra:">Chọn thanh tra:</label>
                  <select name="" id=""
                  onChange={(e)=>{setFormAddInspectorToPlan({...formAddInspectorToPlan,userId:e.target.value})}}
                  >
                              <option value="" disabled selected>Chọn thanh tra:</option>
